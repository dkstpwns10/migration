...
import { studyApi, StudyUpdateRequest, BudgetType } from '@/lib/api';
...
export default function EditStudyPage() {
  ...
  const { data: study, isLoading: studyLoading, error } = useQuery({
  ...
  const updateMutation = useMutation({
    mutationFn: (data: StudyUpdateRequest) => studyApi.updateStudy(studyId, data),
    onSuccess: () => {
  ...
  if (!study.isLeader) {
    return (
      <div className="flex flex-col items-center justify-center py-16">
        ...
        <p className="text-muted-foreground mb-4">스터디장만 스터디를 수정할 수 있습니다.</p>
        <Button onClick={() => router.push(`/studies/${studyId}`)}>스터디로 돌아가기</Button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold">스터디 수정</h1>
          <p className="text-muted-foreground">스터디 정보를 수정합니다</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <Card>
          <CardHeader>
            ...
            <CardDescription>스터디의 기본 정보를 수정해주세요</CardDescription>
          </CardHeader>
        ...
        <Card className="mt-6">
          <CardHeader>
            ...
            <CardDescription>스터디 관련 링크를 수정해주세요</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            {/* Chat URL */}
            <div className="space-y-2">
